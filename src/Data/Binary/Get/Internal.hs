...
runAndCaptureInput :: Get a -> Get (a, B.ByteString)
runAndCaptureInput g = do
    (decoder, bs) <- runAndKeepTrack g
    case decoder of
        Done _ a -> pure (a, fold bs)
        Fail inp s -> C $ \_ _ -> Fail inp s
        _ -> error "Binary: impossible"
...
remaining = C $ \inp ks ->
    let loop acc = Partial $ \minp ->
            case minp of
                Nothing ->
                    let all_inp = B.concat (inp : (reverse acc))
...
"readN/readN merge" forall n m f g.
    apG (readN n f) (readN m g) =
        readN (n + m) (\bs -> f bs $ g (B.unsafeDrop n bs))
    #-}
...
ensureN !n0 = C $ \inp ks -> do
  ...
  where
    -- might look a bit funny, but plays very well with GHC's inliner.
